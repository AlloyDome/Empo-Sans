...
fontInfoList = {
    "versionMajor": 3,
    "versionMinor": 0,
    "fontFileNamePrefix": "EMPOSANS",
    "fontNamePrefix": "Empo Sans",
    "copyrightAdditional": "Copyright 2023 The Empo Sans Project Authors (https://github.com/AlloyDome/Empo-Sans/)"
}
...
def fixStat(font:Path, STAT:str):
    ...
    config[fontInfoList["fontFileNamePrefix"] + "1[wght].ttf"] = config["MPLUS1[wght].ttf"]
    config[fontInfoList["fontFileNamePrefix"] + "2[wght].ttf"] = config["MPLUS2[wght].ttf"]
    gen_stat_tables_from_config(config, fontSTAT)

    for font in fontSTAT:
        font.save(font.reader.file.name)

def step_merge_glyphs_from_ufo(path: Path, instance: ufoLib2.Font, ignore = [], *args) -> None:
    textfile = ""
    for ar in args:
        textfile = ar
    ...
    if textfile:
        ...
        for glyph in ufo:
            if glyph.name not in instance and glyph.name not in ignore:
...
def build_variable(type:str, ds: DesignSpaceDocument) -> None:
    ...
    if type == "one" or type == "two":
        ...
        for instance in ds.instances:
            instance.name = instance.name.replace("M PLUS", fontInfoList["fontNamePrefix"])
            instance.familyName = instance.familyName.replace("M PLUS", fontInfoList["fontNamePrefix"])
            if instance.styleMapFamilyName:
                instance.styleMapFamilyName = str(instance.styleMapFamilyName).replace("M PLUS", fontInfoList["fontNamePrefix"])

        for source in ds.sources:
            if "{" not in source.name:
                step_merge_glyphs_from_ufo(
                    Path("sources/M+1p-"+source.filename[7:-4]+".ufo"), source.font, 
                    ignore = ['uni3005', 'uni3006', 'uni303B', 'zero-han']
                )
                source.familyName = source.familyName.replace("M PLUS", fontInfoList["fontNamePrefix"])
                source.font.info.copyright = source.font.info.copyright + "; " + fontInfoList["copyrightAdditional"]
                
            source.name = source.name.replace("M PLUS", fontInfoList["fontNamePrefix"])
            source.font.info.familyName = source.font.info.familyName.replace("M PLUS", fontInfoList["fontNamePrefix"])
            source.font.info.versionMajor = fontInfoList["versionMajor"]
            source.font.info.versionMinor = fontInfoList["versionMinor"]
        ...
        varFont["OS/2"].ulCodePageRange1 |= 0b0100_0000_0000_0000_0000   # Code page 936（simplified Chinese）
        DSIG_modification(varFont)
        GASP_set(varFont)

        print ("[MPLUS " + type + "] Saving")      
        if type == "one":
            varFont.save(output / (fontInfoList["fontFileNamePrefix"] + "1[wght].ttf"))
            prefix = "MPLUS1"
            fixStat(output / (fontInfoList["fontFileNamePrefix"] + "1[wght].ttf"), "sources/MPLUS_STAT.yaml")
        elif type == "two":
            varFont.save(output / (fontInfoList["fontFileNamePrefix"] + "2[wght].ttf"))
            prefix = "MPLUS2"
            fixStat(output / (fontInfoList["fontFileNamePrefix"] + "2[wght].ttf"), "sources/MPLUS_STAT.yaml")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-W", "--clean", action="store_true", dest="clean", help="Don't remove all source files")
    ...
    if args.sources:
        ...
        for file in sources.glob("*.glyphs"):
            ...
            print("[" + str(file).split(os.sep)[-1] + "] generating source")
